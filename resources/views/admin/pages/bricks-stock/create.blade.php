            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <h5 class="card-title mb-0">Create Bricks Stock</h5>
                        <div>
                            <a href="{{ route('admin.bricks_stock.BricksStock') }}" class="btn btn-sm btn-primary">Bricks
                                Stock</a>
                        </div>
                    </div>
                    <div class="card-body">
                        <form class="row gy-3 needs-validation" method="POST"
                            action="{{ route('admin.bricks_stock.storeBricksStock') }}" novalidate>
                            @csrf
                            ...
                            <div class="col-md-3">
                                ...
                                <select id="bricks_type_category" name="bricks_type_category_id" class="form-control">
                                    ...
                                    @foreach ($category as $value)
                                        <option value="{{ $value->id }}">{{ $value->bricks_type_category_name }}</option>
                                    @endforeach
                                </select>
