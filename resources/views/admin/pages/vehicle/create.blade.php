                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form class="row gy-3 needs-validation" method="POST"
                            action="{{ route('admin.vehicle.storeVehicle') }}" enctype="multipart/form-data" novalidate>
                            @csrf

                            <div class="col-md-2">
                                <label class="form-label">Vehicle Type <span class="text-danger">*</span> </label>
                                <select name="vehicle_type" class="form-control" id="vehicle_type"
                                    onchange="toggleRentAmount()">
                                    <option value="">Select Vehicle Type</option>
                                    <option value="company" {{ old('vehicle_type') == 'company' ? 'selected' : '' }}>Company
                                    </option>
                                    <option value="rent" {{ old('vehicle_type') == 'rent' ? 'selected' : '' }}>Rent
                                    </option>
                                    <option value="customer" {{ old('vehicle_type') == 'customer' ? 'selected' : '' }}>
                                        Customer</option>
                                </select>
                                @error('vehicle_type')
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="text" name="vehicle_name" class="form-control"
                                    placeholder="Enter Vehicle Name" value="{{ old('vehicle_name') }}" required>
                                @error('vehicle_name')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-3" id="rentAmountDiv" style="display: none;">
                                ...
                                <input type="number" name="rent_amount" class="form-control"
                                    placeholder="Enter Rent Amount" value="{{ old('rent_amount') }}">
                                @error('rent_amount')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
